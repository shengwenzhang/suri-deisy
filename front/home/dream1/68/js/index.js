...
function incrementScore(inc) {
	if (isInGame()) {
		...
		renderScoreHud();
		checkScoreAndEndGame(); // 检查分数并结束游戏
	}
}
...
function checkScoreAndEndGame() {
	if (state.game.score > 100) { // 当分数大于500时，结束游戏
	  endGamescore();
	}
}
...
function endGamescore() {
	setActiveMenu(MENU_SCORE);
	handleCanvasPointerUp();
	// 更新最高分数，需要在渲染得分菜单之后执行。
	if (state.game.score > getHighScore()) {
		setHighScore(state.game.score);
	}

	// 在这里添加页面转跳的代码
	window.location.href = "../key.html"; // 替换为目标页面的 URL
}
